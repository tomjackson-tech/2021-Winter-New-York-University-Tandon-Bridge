 ...
 * author: Lin, Kuan-You
 * date: 5/14/2021
 * 2021 NYU Tandon Bridge Homework 17 Question 1

The implementation of the black red tree
*/
...
class RBT {
    ...
    void insertFixUp(RBTNode<T> *&point);
...
//    right-left
    singleCR(point->right); // right rotation
//    right-right
    singleCCR(point); // left rotation
}

// left-right or left-left rotation
template <class T>
void RBT<T>::doubleCR(RBTNode<T> *&point) {
//    left-right
    singleCCR(point->left); // left rotation
//    left-left
    singleCR(point); // right rotation
}
...
template <class T>
void RBT<T>::insertFixUp(RBTNode<T> *&current) {
    // if the parent is black, no need to fix it up
    while (current->parent && (current->parent->color == RED)) {
        // if the parent is a left child of grandparent
        if (current->parent == current->parent->parent->left) {
            ...
            if (uncle && (uncle->color == RED)) {
                ...
                // CASE2: if uncle is BLACK and new node is a right child
                if (current == current->parent->right) {
                    current = current->parent;
                    // left rotation (right-left)
                    singleCCR(current);
                }
                // CASE3: if uncle is BLACK and new node is a left child, no need to rotate
                current->parent->color = BLACK;
                current->parent->parent = RED;
                // (left-right or left-left)
                singleCR(current->parent->parent);
            }
        } else {
            // if the parent is a right child of grandparent
            ...
            if (uncle && (uncle->color == RED)) {
                ...
                // CASE2: if uncle is BLACK and new node is a left child
                if (current == current->parent->left) {
                    current = current->parent;
                    // right rotation (left-right)
                    singleCR(current);
                }
                // CASE3: if uncle is BLACK and new node is a right child
                current->parent->color = BLACK;
                current->parent->parent->color = RED;
                // (right-right or right left)
                singleCCR(current->parent->parent);
            }
        }
    }
    root->color = BLACK;
